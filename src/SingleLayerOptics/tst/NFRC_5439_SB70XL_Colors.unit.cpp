...
private:
    ...
    {
        std::shared_ptr<SpectralAveraging::CSpectralSampleData> aMeasurements_5439 =
    ...
    SingleLayerOptics::CScatteringLayer createXLayer() const
    {
        ...
        auto aSample =
          std::make_shared<SpectralAveraging::CSpectralSample>(aMeasurements, aSolarRadiation);

        aSample->setDetectorData(aDetectorData);
        aSample->setWavelengths(SpectralAveraging::WavelengthSet::Custom, wavelengths);

        double thickness = 3.048e-3;   // [m]
        std::shared_ptr<SingleLayerOptics::CMaterial> aMaterial =

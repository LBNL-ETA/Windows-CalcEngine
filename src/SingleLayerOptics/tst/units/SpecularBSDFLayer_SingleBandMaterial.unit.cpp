...
class TestSpecularBSDFLayer_SingleBandMaterial : public testing::Test
{
private:
...
TEST_F(TestSpecularBSDFLayer_SingleBandMaterial, SingleBandSpecular)
{
    ...
    for(size_t i = 0u; i < correctAbs.size(); ++i)
    {
        EXPECT_NEAR(correctAbs[i], AbsFront[i], 1e-6);
    }
}

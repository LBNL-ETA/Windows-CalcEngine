...
private:
	...
	std::shared_ptr< CSpectralSampleData > loadSampleData_NFRC_VE348() {
		std::shared_ptr< CSpectralSampleData > aMeasurements_VE348 = std::make_shared< CSpectralSampleData >();
...
protected:
	virtual void SetUp() {
		...
		std::shared_ptr< CMaterial > aMaterial = std::make_shared< CMaterialSample >( aSample,
				thickness, MaterialType::Coated, WavelengthRange::Solar );
...
TEST_F( TestVE345ScatteringLayer1, Test1 ) {

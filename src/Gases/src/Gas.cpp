...
namespace Gases
{
    ...
    GasProperties CGas::getSimpleGasProperties(const double alpha1, const double alpha2)
    {
        m_SimpleProperties = m_GasItem[0].getFractionalGasProperties(alpha1, alpha2);
        for(auto it = next(m_GasItem.begin()); it != m_GasItem.end(); ++it)
        {
            m_SimpleProperties += it->getFractionalGasProperties(alpha1, alpha2);
        }

        return m_SimpleProperties;
    }

    GasProperties CGas::getGasProperties(const double alpha1, const double alpha2)
    {
        auto aSettings = CGasSettings::instance();
        return aSettings.getVacuumPressure() < m_Pressure
                 ? getStandardPressureGasProperties(alpha1, alpha2)
                 : getVacuumPressureGasProperties(alpha1, alpha2);
    }

    GasProperties CGas::getStandardPressureGasProperties(const double alpha1, const double alpha2)
    {
        auto simpleProperties = getSimpleGasProperties(alpha1, alpha2);
        ...
        for(size_t i = 0; i < gasSize; ++i)
        {
            ...
            cpMix += itGasProperties.m_SpecificHeat * m_GasItem[i].fraction()
    ...
    GasProperties CGas::getVacuumPressureGasProperties(double alpha1, double alpha2)
    {
        return getSimpleGasProperties(alpha1, alpha2);
    }

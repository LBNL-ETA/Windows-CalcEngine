...
namespace FenestrationCommon {
	...
	CMatrixSeries & CMatrixSeries::operator=( CMatrixSeries const & t_MatrixSeries ) {
		...
		for ( size_t i = 0; i < m_Size1; ++i ) {
			m_Matrix[ i ].resize( m_Size2 );
			for ( size_t j = 0; j < m_Size2; ++j ) {
				m_Matrix[ i ][ j ] = std::unique_ptr< CSeries >(
						new CSeries( *t_MatrixSeries.m_Matrix[ i ][ j ] ) );
			}
		}
		return *this;
	}
	...
	void
	CMatrixSeries::integrate( const IntegrationType t_Integration, double normalizationCoefficient ) {
		for ( size_t i = 0; i < m_Matrix.size(); ++i ) {
			for ( size_t j = 0; j < m_Matrix[ i ].size(); ++j ) {
	...
	std::shared_ptr< std::vector< std::shared_ptr< std::vector< double > > > > CMatrixSeries::getSums(
		...
		std::shared_ptr< std::vector< std::shared_ptr< std::vector< double > > > > Result =
				std::make_shared< std::vector< std::shared_ptr< std::vector< double > > > >(
						m_Matrix.size() );
		for ( size_t i = 0; i < m_Matrix.size(); ++i ) {

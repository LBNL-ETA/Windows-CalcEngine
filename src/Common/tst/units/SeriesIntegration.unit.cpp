...
public:
    [[nodiscard]] CSeries getProperty() const
    {
        return m_Series;
    }
};
...
TEST_F(TestSeriesIntegration, TestRectangular)
{
    ...
    const auto & aSpectralProperties = getProperty();

    const auto aIntegratedProperties = aSpectralProperties.integrate(IntegrationType::Rectangular);

    const std::vector<double> correctResults = {
      0.0027555, 0.0027595, 0.0027615, 0.0027645, 0.0027715, 0.0027760, 0.0027895,
      0.0028130, 0.0028495, 0.0028945, 0.0029420, 0.0029745, 0.0029855, 0.0029730,
      0.0029425, 0.0028920, 0.0028330, 0.0027735, 0.0027285, 0.0027125};
...
TEST_F(TestSeriesIntegration, TestTrapezoidal)
{
    ...
    const auto aIntegratedProperties = aSpectralProperties.integrate(IntegrationType::Trapezoidal);

    const std::vector<double> correctResults{
      0.00275750, 0.00276050, 0.00276300, 0.00276800, 0.00277375, 0.00278275, 0.00280125,
      0.00283125, 0.00287200, 0.00291825, 0.00295825, 0.00298000, 0.00297925, 0.00295775,
      0.00291725, 0.00286250, 0.00280325, 0.00275100, 0.00272050, 0.00271500};
...
TEST_F(TestSeriesIntegration, TestRectangularCentroid)
{
    ...
    const auto aIntegratedProperties =
      aSpectralProperties.integrate(IntegrationType::RectangularCentroid, 2);

    const std::vector<double> correctResults{0.0013777, 0.0013798, 0.0013807, 0.0013823, 0.0013857,
...
TEST_F(TestSeriesIntegration, TestPreWeighted)
{
    ...
    const std::vector<double> correctResults{
      0.275550, 0.275950, 0.276150, 0.276450, 0.277150, 0.277600, 0.278950,
      0.281300, 0.284950, 0.289450, 0.294200, 0.297450, 0.298550, 0.297300,
      0.294250, 0.289200, 0.283300, 0.277350, 0.272850, 0.271250, 0.271750};
...
TEST_F(TestSeriesIntegration, IntegrationWithInterpolation)
{
    const auto & aSpectralProperties = getProperty();
    const std::vector<double> integrationPoints{0.5, 0.55, 0.6};
    constexpr auto normalizationCoefficient{1};

    const auto aIntegratedProperties = aSpectralProperties.integrate(
      IntegrationType::Rectangular, normalizationCoefficient, integrationPoints);

    const std::vector<double> correctResults = {0.027555, 0.02942};

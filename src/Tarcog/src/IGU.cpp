...
namespace Tarcog
{
    ...
    void CIGU::setDeflectionProperties(double const t_Tini, double const t_Pini)
    {
        ...
        for(auto & aLayer : getSolidLayers())
        {
            // Deflection could also be decorated (created) outside in which case program already
            // have a layer as deflection layer. If that is not done then layer must be decorated
            // with default deflection properties

...
namespace Tarcog
{
    ...
    double CNusseltNumber60::calculate(double const, double const t_Ra, double const t_Asp)
    {
        ...
        const double Nu1 =
          pow(1 + pow(0.0936 * pow(t_Ra, 0.314) / (1 + G), 7), 0.1428571);   // equation 45
    ...
    double
      CNusseltNumber60To90::calculate(double const t_Tilt, double const t_Ra, double const t_Asp)
    {
    ...
    double
      CNusseltNumber90to180::calculate(double const t_Tilt, double const t_Ra, double const t_Asp)
    {
        std::shared_ptr<CNusseltNumber90> nusselt90 = std::make_shared<CNusseltNumber90>();
        const double Nu90 = nusselt90->calculate(t_Tilt, t_Ra, t_Asp);
    ...
    double
      CNusseltNumber::calculate(double const t_Tilt, double const t_Ra, double const t_Asp) const
    {
        using ConstantsData::PI;

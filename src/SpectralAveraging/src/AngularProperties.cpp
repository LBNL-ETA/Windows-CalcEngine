...
namespace SpectralAveraging
{
    ...
    CAngularProperties::CAngularProperties(double const t_TransmittanceZero,
        ...
        m_Transmittance0(t_TransmittanceZero),
        m_Reflectance0(t_ReflectanceZero),
        m_Transmittance(-1),
        m_Reflectance(-1),
        m_StateAngle(-1),
        m_StateWavelength(-1)
    ...
    double CAngularPropertiesUncoated::transmittance(double const t_Angle,
    ...
    {
        CAngularProperties::checkStateProperties(
          t_Angle,
    ...
    Coefficients::Coefficients(double const t_C0,
                               double const t_C1,
                               double const t_C2,
                               double const t_C3,
                               const double t_C4) :
    ...
    CAngularPropertiesFactory::CAngularPropertiesFactory(double const t_Transmittance0,
        ...
        m_Transmittance0(t_Transmittance0),
        m_Reflectance0(t_Reflectance0),
      ...
      CAngularPropertiesFactory::getAngularProperties(SurfaceType const t_SurfaceType)
    {
        ...
        switch(t_SurfaceType)
        {
            case SurfaceType::Coated:
                aProperties = std::make_shared<CAngularPropertiesCoated>(
                  m_Transmittance0, m_Reflectance0, m_SolarTransmittance0);
                break;
            case SurfaceType::Uncoated:
                aProperties = std::make_shared<CAngularPropertiesUncoated>(
                  m_Thickness, m_Transmittance0, m_Reflectance0);
                break;
            default:
                throw std::runtime_error(

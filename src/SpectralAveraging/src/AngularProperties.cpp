...
namespace SpectralAveraging {
	...
	CAngularProperties::CAngularProperties( double const t_TransmittanceZero,
	...
	void CAngularProperties::checkStateProperties( double const t_Angle, double const ) {
		if( t_Angle > 90 || t_Angle < 0 ) {
			throw std::runtime_error(
					"Incoming angle is out of range. Incoming angle must be between 0 and 90 degrees." );
		}
	...
	double
	CAngularPropertiesUncoated::transmittance( double const t_Angle, double const t_Wavelength ) {
		checkStateProperties( t_Angle, t_Wavelength );
		return m_Transmittance;
	}

	double
	CAngularPropertiesUncoated::reflectance( double const t_Angle, double const t_Wavelength ) {
		checkStateProperties( t_Angle, t_Wavelength );
	...
	CAngularPropertiesCoated::CAngularPropertiesCoated( const double t_Transmittance,
	                                                    const double t_Reflectance,
	                                                    const double t_SolarTransmittance ) :
			CAngularProperties( t_Transmittance, t_Reflectance ),
			m_SolTransmittance0( t_SolarTransmittance ) {

	}

	double
	CAngularPropertiesCoated::transmittance( double const t_Angle, double const t_Wavelength ) {
		checkStateProperties( t_Angle, t_Wavelength );
		return m_Transmittance;
	}
	...
	void CAngularPropertiesCoated::checkStateProperties( double const t_Angle, double const ) {
		CAngularProperties::checkStateProperties( t_Angle,
				0 ); // Wavelength is not provided for coated glass
	...
	Coefficients::Coefficients( double const t_C0, double const t_C1, double const t_C2,
	                            double const t_C3, const double t_C4 ) :
			...
			case CoatingProperty::T:
				switch( t_Type ) {
					...
					case CoatingType::Bronze:
						aCoefficients = std::make_shared< Coefficients >( -0.002, 2.813, -2.341, -0.05725,
								0.599 );
						break;
					default:
	...
	CAngularPropertiesFactory::CAngularPropertiesFactory( double const t_Transmittance0,
	                                                      double const t_Reflectance0,
	                                                      double const t_Thickness,
	                                                      double const t_SolarTransmittance ) :
			m_Thickness( t_Thickness ), m_Transmittance0( t_Transmittance0 ),
			m_Reflectance0( t_Reflectance0 ), m_SolarTransmittance0( t_SolarTransmittance ) {

	}

	std::shared_ptr< CAngularProperties >
	CAngularPropertiesFactory::getAngularProperties( SurfaceType const t_SurfaceType ) {
		std::shared_ptr< CAngularProperties > aProperties = nullptr;
		switch( t_SurfaceType ) {
			case SurfaceType::Coated:
				aProperties = std::make_shared< CAngularPropertiesCoated >( m_Transmittance0,
						m_Reflectance0, m_SolarTransmittance0 );
				break;
			case SurfaceType::Uncoated:
				aProperties = std::make_shared< CAngularPropertiesUncoated >( m_Thickness, m_Transmittance0,
						m_Reflectance0 );
				break;
			default:
				throw std::runtime_error(
						"Incorrect surface type. Cannot create correct angular properties." );
				break;
		}

...
TEST_F(MultiPaneBSDF_20243_103, TestSpecular1)
{
    ...
    EXPECT_NEAR(0.179420753, rhoDiff, 1e-6);

    double absDiff1 = aLayer.AbsDiff(minLambda, maxLambda, Side::Front, 1);
    EXPECT_NEAR(0.820340656, absDiff1, 1e-6);
    ...
    EXPECT_NEAR(0.131814317, rhoHem, 1e-6);

    double rhoDir =
      aLayer.DirDir(minLambda, maxLambda, Side::Front, PropertySurface::R, theta, phi);
    EXPECT_NEAR(0.131814317, rhoDir, 1e-6);

    double abs1 = aLayer.Abs(minLambda, maxLambda, Side::Front, 1, theta, phi);
    EXPECT_NEAR(0.867718234, abs1, 1e-6);
    ...
    EXPECT_NEAR(0.134687647, rhoHem, 1e-6);

    abs1 = aLayer.Abs(minLambda, maxLambda, Side::Front, 1, theta, phi);
    EXPECT_NEAR(0.865013937, abs1, 1e-6);

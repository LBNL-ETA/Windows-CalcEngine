...
// Example on how to create scattered multilayer when only visible and solar spectrum are provided.

class MultiPaneScattered_DualMaterial : public testing::Test
{
private:
...
protected:
    virtual void SetUp()
    {
        ...
        const auto Tsol = 0.7;
        const auto Rfsol = 0.1;
        const auto Rbsol = 0.1;
        ...
        CScatteringLayer Layer = CScatteringLayer::createSpecularLayer(aMaterialDual);

        // Equivalent scattering layer
        m_Layer = CMultiLayerScattered::create(Layer);
...
TEST_F(MultiPaneScattered_DualMaterial, TestSpecular1)
{
    ...
    EXPECT_NEAR(0.7, T_dir_dir, 1e-6);

...
// Test equivalent properties of double layer with direct-direct, direct-diffuse and diffuse-diffuse
// components Tests include adding layer on back and front sides
...
protected:
    virtual void SetUp()
    {
        CScatteringSurface aFront(0.06, 0.04, 0.46, 0.12, 0.46, 0.52);
        CScatteringSurface aBack(0.11, 0.26, 0.34, 0.19, 0.64, 0.22);
        CScatteringLayer aLayer1(aFront, aBack);

        aFront = CScatteringSurface(0.1, 0.05, 0.48, 0.26, 0.56, 0.34);
        aBack = CScatteringSurface(0.15, 0, 0.38, 0.19, 0.49, 0.39);
        CScatteringLayer aLayer2(aFront, aBack);
...
TEST_F(TestMultiLayer1, TestTripleLayerFront)
{
    ...
    double Tf =
      aLayer->getPropertySurface(minLambda, maxLambda, PropertySurface::T, aSide, aScattering);
    EXPECT_NEAR(0.000486418, Tf, 1e-6);

    double Rf =
      aLayer->getPropertySurface(minLambda, maxLambda, PropertySurface::R, aSide, aScattering);
    ...
    Tf = aLayer->getPropertySurface(minLambda, maxLambda, PropertySurface::T, aSide, aScattering);
    EXPECT_NEAR(0.190095209, Tf, 1e-6);

    Rf = aLayer->getPropertySurface(minLambda, maxLambda, PropertySurface::R, aSide, aScattering);
...
TEST_F(TestMultiLayer1, TestTripleLayerBack)
{
    ...
    Tb = aLayer->getPropertySurface(minLambda, maxLambda, PropertySurface::T, aSide, aScattering);
    EXPECT_NEAR(0.219867246, Tb, 1e-6);

    Rb = aLayer->getPropertySurface(minLambda, maxLambda, PropertySurface::R, aSide, aScattering);
...
TEST_F(TestMultiLayer1, TestFrontSideAbsorptances)
{
    ...
    double Af1_dir = aLayer->getAbsorptanceLayer(minLambda, maxLambda, 1, aSide, aScattering);
    EXPECT_NEAR(0.362217125, Af1_dir, 1e-6);

    double Af2_dir = aLayer->getAbsorptanceLayer(minLambda, maxLambda, 2, aSide, aScattering);
    EXPECT_NEAR(0.08499287, Af2_dir, 1e-6);

    double Af3_dir = aLayer->getAbsorptanceLayer(minLambda, maxLambda, 3, aSide, aScattering);
    ...
    double Af1_dif = aLayer->getAbsorptanceLayer(minLambda, maxLambda, 1, aSide, aScattering);
    EXPECT_NEAR(0.057730707, Af1_dif, 1e-6);

    double Af2_dif = aLayer->getAbsorptanceLayer(minLambda, maxLambda, 2, aSide, aScattering);
    EXPECT_NEAR(0.074691415, Af2_dif, 1e-6);

    double Af3_dif = aLayer->getAbsorptanceLayer(minLambda, maxLambda, 3, aSide, aScattering);

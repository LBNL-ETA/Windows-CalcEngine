...
namespace MultiLayerOptics
{
    ...
    std::shared_ptr<CMatrixSeries> CEquivalentBSDFLayer::getTotal(const Side t_Side, const PropertySimple t_Property)
    {
        if(!m_Calculated)
        {
            calculate();
    ...
    void CEquivalentBSDFLayer::calculateWavelengthProperties(size_t const t_NumOfLayers, size_t const t_Start, size_t const t_End)
    {
        for(auto i = t_Start; i < t_End; ++i)
        {
            ...
            for(auto aSide : EnumSide())
            {
                ...
                {
                    m_TotA.at(aSide)->addProperties(k, curWL, m_LayersWL[i].getLayerAbsorptances(k + 1, aSide));
                }

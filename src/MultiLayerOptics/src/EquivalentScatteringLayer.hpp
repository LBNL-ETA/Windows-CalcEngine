...
namespace MultiLayerOptics
{
    ...
    // Calculates Transmittance and reflectance of multi layer IGU with direct and diffuse
    // properties
    class CEquivalentScatteringLayer
    {
    public:
        CEquivalentScatteringLayer(const double Tf_dir_dir,
        ...
        void addLayer(const double Tf_dir_dir,
                      const double Rf_dir_dir,
                      const double Tb_dir_dir,
                      const double Rb_dir_dir,
                      const double Tf_dir_dif,
    ...
    private:
        ...
        static double getInterreflectance(const SingleLayerOptics::CScatteringSurface & t_First,
                                          const SingleLayerOptics::CScatteringSurface & t_Second,
                                          const FenestrationCommon::Scattering t_Scattering);

        // Add diffuse and direct components from scattering layer properties
        void addLayerComponents(SingleLayerOptics::CScatteringLayer & t_Layer,
                                const FenestrationCommon::Side t_Side,
                                const double t_Theta = 0,
                                const double t_Phi = 0) const;

        std::shared_ptr<SimpleResults>
          calcDirectDiffuseTransAndRefl(const SingleLayerOptics::CScatteringSurface & f1,

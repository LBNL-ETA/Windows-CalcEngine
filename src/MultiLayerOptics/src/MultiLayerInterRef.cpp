...
namespace MultiLayerOptics {
	...
	CInterRef::CInterRef( CScatteringLayer & t_Layer, const double t_Theta, const double t_Phi ) :
		...
		for ( Side aSide : EnumSide() ) {
			m_Abs[ std::make_pair( aSide, ScatteringSimple::Diffuse ) ] = std::vector< double >();
			m_Abs[ std::make_pair( aSide, ScatteringSimple::Direct ) ] = std::vector< double >();
		}
	}

	void CInterRef::addLayer( CScatteringLayer & t_Layer, const Side t_Side,
	...
	void CInterRef::createForwardLayers( const double t_Theta, const double t_Phi ) {
		...
		CScatteringSurface aFront( 1, 0, 0, 0, 1, 0 );
		CScatteringSurface aBack( 1, 0, 0, 0, 1, 0 );
		CScatteringLayer exterior( aFront, aBack );

...
namespace Viewer {
  ...
  public:
    ...
    std::shared_ptr< const CViewSegment2D > getClosestCommonSegment( 
      std::shared_ptr< const CDirect2DBeam > t_Beam ) const;

    double cosAngle( std::shared_ptr< const CViewSegment2D > t_Segment );

  private:
    // Checks if segment is aleardy part of beam hit
    bool isSegmentIn( std::shared_ptr< const CViewSegment2D > t_Segment ) const;
  ...
  class CDirect2DRay {
  public:
    CDirect2DRay( std::shared_ptr< CDirect2DBeam > t_Beam1, std::shared_ptr< CDirect2DBeam > t_Beam2 );
    CDirect2DRay( std::shared_ptr< CViewSegment2D > t_Ray1, std::shared_ptr< CViewSegment2D > t_Ray2 );
  ...
  public:
    CDirect2DRaysResult( const double t_ProfileAngle, double const t_DirectToDirect, 
      std::shared_ptr< std::vector < BeamViewFactor > > t_BeamViewFactors );
  ...
  private:
    ...
    bool isInRay( std::shared_ptr< const CPoint2D > t_Point );

    std::shared_ptr< CViewSegment2D > createSubBeam( std::shared_ptr< const CPoint2D > t_Point, 
